...
def ssd_bboxes_select(predictions_net,
    ...
    Return:
      classes, scores, bboxes: Numpy arrays...
    """
    scores = r_scores[range(predictions_net.shape[0]), predictions_net]
    sub_scores = scores
    idxes = np.where(sub_scores > select_threshold)
    classes = predictions_net[idxes]
    scores = sub_scores[idxes]
    bboxes = localizations_net[idxes]

    ##
    # idxes = np.where((classes > 0) + ((scores < 0.99) * (classes == 0)))
    idxes = np.where(classes > 0)
...
def ssd_bboxes_selectN(
                        rclasses,
                        rscores,
                        rbboxes,
                        select_threshold=0.5):

    idxes = np.where(rscores > select_threshold)
    classes = rclasses[idxes]
    scores = rscores[idxes]
    bboxes = rbboxes[idxes]
